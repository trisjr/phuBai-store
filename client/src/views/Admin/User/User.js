...
function User() {
    ...
    const [sortState, setSort, sort] = useSort("email");
    ...
    const updateRoleUser = async (user) => {
        if (user.role !== "ADMIN") {
            const { success, message } = await updateUser(user._id, {
                ...user,
                role: user.role === "DEV" ? "USER" : "DEV",
            });
            if (!success) {
                console.log(message);
            }
                        ...
                        <div className={PageStyles.table}>
                            <div className={PageStyles.table__head}>
                                ...
                                <div
                                    ...
                                    onClick={() => handleSetSort("email")}
                                >
                                    <div className={Styles.text}>Email</div>
                                    <i className={PageStyles.icon}>
                                        {sortState.value === "email" ? (
                                            sortState.type === "asc" ? (
                                                <IoCaretDown
    ...
    return (
        <div className={clsx(PageStyles.content, PageStyles[theme])}>
            <div className={PageStyles.title}>User</div>
            {body}
        </div>
    );
}
